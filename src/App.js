...
function Container() 
{
  ...
  let obj={history,data,setData,setUser,user}
  ...
  let themebutton=<IconButton onClick={()=>setThemechange((themechange==='dark')?'light':'dark')}>{mode}</IconButton>
  ...
  const Search=(searchData)=>{
    ...
    if(searchData!=="")
    {
      const filterData = user.filter(({ Name }) => {
  ...
  return (
                ...
                <Fragment key={1} >
                 ...
                 {themebutton}   
                  ...
                  {/* Search Field */}
                 <InputBase  placeholder="Search" onChange={(e)=>Search(e.target.value)} className="searchfield" style={{backgroundColor:(themechange==='light')&&"white"}}
...
{ 
  ...
  const {data,setData,setUser,user}=useContext(context);
  let history=useHistory();

// Getting Data using Axios
const getdata=()=>{
axios.get(`${URL}/userlist`).then((response)=>{setData(response.data);setUser(response.data)})
}   

useEffect(getdata,[setData,setUser])
...
const deleteuser=(_id)=>{
  ...
  return(<div className="displayuserlist">
            ...
            <Button onClick={()=>history.push('/Adduser')} variant='contained' color='warning'>Add User</Button>
        ...
        {/* Table Body */}
        { data.length && Array.isArray(data) ? (data.map(({Name,Avatar,Mobile,Mail,Status,_id})=>{ return(
        ...
        <TableCell align="center"> <Features deleteuser={deleteuser}  id={_id} /></TableCell>
          ...
          {(!data.length && Array.isArray(data)) ?  <CircularProgress id='progress' />:(user.length && data==='')?<p style={{textAlign:'center'}}>User Not Found</p>:''}
        {/* Navigate Button */}
...
function Adduserlist()
{
  ...
  // Using Axios Add user to the database
...
// Get individual userdata using Axios
useEffect(()=>{
axios({url:`${URL}/userlist/${id}`,method:'GET'}).then((response)=>setUser(response.data))},[id])
...
function Updateuser({open,user,id,handleClickOpen,setCount})
{
  ...
  const{Name,Avatar,Mobile,Mail}=user  // Destruturing from old user data

  handleClickOpen() // Opens the dialogbox


// Update user data using Axios
const updateuser=(updateduser)=>{
axios({url:`${URL}/userlist/${id}`,
method:"PUT",
data:updateduser,
}).then(()=>setCount(0)).then(()=> getdata())
}

// Get data after the update
const getdata=()=>{
  axios.get(`${URL}/userlist`).then((response)=>setData(response.data))
}   
